...
pub fn handle_time_action(state: &mut TimeInputState, action: TimeAction) {
    match action {
        TimeAction::Digit(d) if d <= 9 => {
            if let Some(first) = state.digit_buf.take() {
                ...
                match state.focused_field {
                    TimeField::Hours => {
                        state.hours = val.min(23);
                        state.focused_field = TimeField::Minutes;
                    }
                    TimeField::Minutes => {
                        state.minutes = val.min(59);
                    }
                }
            } else {
        ...
        TimeAction::NextField => {
            state.digit_buf = None;
            state.focused_field = TimeField::Minutes;
        }
        TimeAction::PrevField => {
            state.digit_buf = None;
            state.focused_field = TimeField::Hours;
        }
        _ => {}
    }
}
