...
impl Grid {
    ...
    pub fn set(&mut self, addr: CellAddr, value: CellValue) {
        ...
        let current_cols = if self.cells.is_empty() { 0 } else { 
...
mod tests {
    ...
    fn test_celladdr_parsing() {
        assert_eq!("A1".parse::<CellAddr>().unwrap(), CellAddr { col: 0, row: 0 });
        assert_eq!("B3".parse::<CellAddr>().unwrap(), CellAddr { col: 1, row: 2 });
        assert_eq!("AA1".parse::<CellAddr>().unwrap(), CellAddr { col: 26, row: 0 });
        assert_eq!("Z1".parse::<CellAddr>().unwrap(), CellAddr { col: 25, row: 0 });
    ...
    fn test_cellvalue_display() {
        ...
        assert_eq!(CellValue::Error(CellError::DivByZero).to_string(), "#DIV/0!");
